...
Backbone.ModalView = Backbone.View.extend({
    ...
    showModalBlanket: function() {
        return this.ensureModalBlanket().fadeIn(this.fadeInDuration);
    },
    hideModalBlanket: function() {
        return this.modalBlanket.fadeOut(this.fadeOutDuration);
    },
    ensureModalContainer: function(target) {
        if (target != null) {
            // A target is passed in, we need to re-render the modal container into the target.
            if (this.modalContainer != null) {
                this.modalContainer.detach();
                this.modalContainer = null;
            }
        }
        if (this.modalContainer == null) {
            this.modalContainer = $("<div id='"+this.containerID+"'>").css({
                "z-index": "99999",
                "position": "relative",
            }).appendTo(target);
        }
    ...
    ensureModalBlanket: function() {
        ...
        if (this.modalBlanket.length == 0) {
            this.modalBlanket = $("<div id='modal-blanket'>").css({
                ...
                top: 0,
                left: 0,
                height: '10000px', // Span the full document height...
    ...
    click: function(event) {
        if (event.target.id == "modal-blanket" && this.defaultOptions.backgroundClickClosesModal) {
            this.hideModal();
        }
    },
    ...
    hideModal: function(callback) {
        ...
        $(this.modalContainer).fadeOut(this.fadeOutDuration, function() {
            container.remove();
    ...
    getCoordinate: function(coordinate, css) {
        if (typeof(css[coordinate]) !== "undefined") {
            ...
            delete css[coordinate]; // Don't apply positioning to the $el, we apply it to the modal container. detach it from options.css
            return value;
        }
    },
    recenter: function() {
    ...
    showModal: function(options) {
        ...
        this.modalContainer.animate(animateProperties, this.fadeInDuration);
        return this;
    }
});